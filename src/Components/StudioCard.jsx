  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-base sm:text-xl md:text-2xl font-bold">Studio</h1>
        <button
          onClick={() => {
            navigate("/services/podcast");
      ...
      <div className="relative">
        {showLeftArrow && (
          <button
            onClick={scrollLeft}
            className="absolute left-0 z-10 p-2 bg-gray-200 rounded-full shadow hover:bg-gray-300 transition transform -translate-y-1/2 top-1/2"
          >
            ❮
          </button>
        )}
        ...
        <div
          ...
          className="flex overflow-x-auto scrollbar-hide gap-3 sm:gap-6 md:gap-8"
        >
          {cardData.map((card, index) => (
            <div
              key={index}
              onClick={() => {
                navigate(`services/${card.category.toLowerCase()}`);
                window.scrollTo(0, 50);
              }}
            ...
            >
              ...
              <div className="p-2 sm:p-4 flex flex-col items-center justify-between flex-grow">
                <h2 className="text-sm sm:text-lg md:text-xl font-semibold text-gray-800">
                  {card.category}
                </h2>
                <div className="w-full mt-2 sm:mt-3 flex justify-center opacity-100 group-hover:opacity-100 transition-opacity">
                  <button className="w-[75%] sm:w-[60%] text-center text-white bg-gradient-to-r from-purple-600 to-blue-600 hover:opacity-90 rounded-md p-1 sm:p-2 md:p-3 text-xs sm:text-sm md:text-base">
                    Watch Now
                  </button>
                </div>
              </div>
        ...
        {showRightArrow && (
          <button
            onClick={scrollRight}
            className="absolute right-0 z-10 p-2 bg-gray-200 rounded-full shadow hover:bg-gray-300 transition transform -translate-y-1/2 top-1/2"
          >
            ❯
          </button>
        )}
